<template>
    <div class="panel">
        ...
        <input placeholder="工号（选填）" maxlength="32" autocomplete="new-password" class="panel-ipt" type="text" v-model="workId" />
        <input placeholder="真实姓名" maxlength="32" autocomplete="new-password" class="panel-ipt" type="text" v-model="realName" />
        <p class="link-box clearfix">
            <a href="login.html" class="to-link">返回登录</a>
            <a href="getPassword.html" class="to-link fl-r">找回密码</a>
        </p>
        <a href="javascript:void(0);" class="panel-button" @click="register">注册</a>
    </div>
</template>
<script>
        ...
        methods: {
            ...
            async register() {
                ...
                if (this.realName.length <= 0) {
                    Util.toast('请输入真实姓名', 'notice', 1500);
                    return false;
                }
                try {
                    const { status } = await UserlAPI.register({
                        mobile: this.userName,
                        pwd: this.password,
                        role: 'crm',
